...
| 2 | Spectral target (first gap → all gaps → response bins → response + intervention) | **Half done** (2026-09-19). On their common audited population, 303 records / 22 parents, `d2` beats `gap_inverse_square` by **−0.01608 [−0.02293, −0.00962]** and at every runtime separately; the raw first-gap rule is **worse than linear** by +0.03242 [+0.00112, +0.06408]. So transition-element resolution adds decision value. Response bins and response-plus-intervention as *learning targets* remain untested. [Report](../reports/spectral_ladder_2026-09-19/SPECTRAL_LADDER.md) |
