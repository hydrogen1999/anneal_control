...
| 2 | Spectral target (first gap → all gaps → response bins → response + intervention) | **Three of four rungs done** (rung 3 added 2026-09-20). Rungs 1–2, as *teachers*: on their common audited population, 303 records / 22 parents, `d2` beats `gap_inverse_square` by **−0.01608 [−0.02293, −0.00962]** at every runtime, and the raw first-gap rule is **worse than linear** by +0.03242 [+0.00112, +0.06408]. Rung 3, as a *learning target*: eight response bins against three moments is **+0.00001 [−0.00104, +0.00118], Holm p = 0.9937** over 48 held-out parents and five seeds — a null tight enough to exclude 2 % of the method's own effect. The auxiliary loss itself is also null (+0.00055), replicating Factor 5 on a fresh dataset. **Resolution matters when the spectral object is the control rule and not when it is a supervision target.** Rung 4 needs intervention labels attached to these records and remains open. [Rungs 1–2](../reports/spectral_ladder_2026-09-19/SPECTRAL_LADDER.md), [rung 3](../reports/spectral_rung3_2026-09-20/RUNG3.md) |
